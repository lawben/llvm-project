...
AArch64TargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case Intrinsic::aarch64_neon_uabd: {
    unsigned Opcode =
        IntNo == Intrinsic::aarch64_neon_uabd ? ISD::ABDU : ISD::ABDS;
    return DAG.getNode(Opcode, dl, Op.getValueType(), Op.getOperand(1),
                       Op.getOperand(2));
  }
...
combineVectorCompareAndBitcast(SDNode *N, TargetLowering::DAGCombinerInfo &DCI,
  ...
  // bits before bitcasting. This op can be skipped if we replace the use
  // chain. We are defensive here to avoid producing wrong code in case we
  // are not aware of the bitcast pattern.
...
static SDValue performSETCCCombine(SDNode *N,
  ...
  if (SDValue V = combineVectorCompareAndBitcast(N, DCI, DAG))
    return V;
